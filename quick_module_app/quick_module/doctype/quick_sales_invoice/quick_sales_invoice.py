# Copyright (c) 2024, kaushal khute and contributors
# For license information, please see license.txt

# import frappe
from frappe.model.document import Document

class QuickSalesInvoice(Document):
	pass
...
class QuickSalesInvoice(SellingController):
	def __init__(self, *args, **kwargs):
		super(QuickSalesInvoice, self).__init__(*args, **kwargs)
		self.status_updater = [
			{
				"source_dt": "Quick Sales Invoice Item",
	...
	def validate(self):
		# super(QuickSalesInvoice, self).validate()
	...
	def validate_for_repost(self):
		...
		self.validate_account_for_change_amount()
		self.validate_income_account()
		validate_docs_for_voucher_types(["Quick Sales Invoice"])
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				...
				if self.doctype == "Quick Sales Invoice" and self.docstatus == 1:
	...
	def on_submit(self):
		...
		if (
			...
			against_si_doc = frappe.get_doc("Quick Sales Invoice", self.return_against)
	...
	def check_if_consolidated_invoice(self):
		...
		if self.doctype == "Quick Sales Invoice" and self.is_consolidated:
	...
	def before_cancel(self):
		...
		super(QuickSalesInvoice, self).before_cancel()
		self.update_time_sheet(None)

	def on_cancel(self):
		check_if_return_invoice_linked_with_payment_entry(self)

		super(QuickSalesInvoice, self).on_cancel()
			...
			self.status_updater.append(
				{
					...
					"extra_cond": """ and exists(select name from `tabQuick Sales Invoice`
					where name=`tabQuick Sales Invoice Item`.parent and update_stock = 1)""",
				}
			)
				...
				self.status_updater.append(
					{
						...
						"second_join_field": "so_detail",
						"extra_cond": """ and exists (select name from `tabQuick Sales Invoice` where name=`tabQuick Sales Invoice Item`.parent and update_stock=1 and is_return=1)""",
					}
				)
	...
	def set_missing_values(self, for_validate=False):
		...
		super(QuickSalesInvoice, self).set_missing_values(for_validate)
	...
	def validate_with_previous_doc(self):
		super(QuickSalesInvoice, self).validate_with_previous_doc(
			{
				"Sales Order": {
	...
	def validate_warehouse(self):
		super(QuickSalesInvoice, self).validate_warehouse()
	...
	def make_item_gl_entries(self, gl_entries):
				...
				if item.is_fixed_asset:
					...
					if self.is_return:
						...
						if asset.calculate_depreciation:
							posting_date = frappe.db.get_value("Quick Sales Invoice", self.return_against, "posting_date")
		...
		if cint(self.update_stock) and erpnext.is_perpetual_inventory_enabled(self.company):
			gl_entries += super(QuickSalesInvoice, self).get_gl_entries()
	...
	def make_gle_for_rounding_adjustment(self, gl_entries):
		if (
			...
			round_off_account, round_off_cost_center = get_round_off_account_and_cost_center(
				self.company, "Quick Sales Invoice", self.name, self.use_company_roundoff_cost_center
			)
		...
		for d in self.get("items"):
			if d.dn_detail:
				billed_amt = frappe.db.sql(
					"""select sum(amount) from `tabQuick Sales Invoice Item`
	...
	def update_serial_no(self, in_cancel=False):
		"""update Quick Sales Invoice refrence in Serial No"""
	...
	def validate_serial_numbers(self):
		"""
		validate serial number agains Delivery Note and Quick Sales Invoice
		"""
	...
	def validate_serial_against_delivery_note(self):
		"""
		validate if the serial numbers in Quick Sales Invoice Items are same as in
		Delivery Note Item
		"""
		...
		if not status:
			if self.docstatus == 2:
				...
				# Check if outstanding amount is 0 due to credit note issued against invoice
					...
					"Quick Sales Invoice", {"is_return": 1, "return_against": self.name, "docstatus": 1}
				):
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if doctype in ["Quick Sales Invoice", "Sales Order"]:
		partytype, ref_partytype, internal = "Customer", "Supplier", "is_internal_customer"

		if doctype == "Quick Sales Invoice":
		...
		if doctype == "Purchase Invoice":
			ref_doc = "Quick Sales Invoice"
		else:
			ref_doc = "Sales Order"

	if inter_company_reference:
		doc = frappe.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Quick Sales Invoice", "Sales Order"] else doc.customer
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if doctype in ["Quick Sales Invoice", "Purchase Invoice"]:
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if doctype in ["Quick Sales Invoice", "Purchase Invoice"]:
		ref_doc = "Purchase Invoice" if doctype == "Quick Sales Invoice" else "Quick Sales Invoice"
		ref_field = "inter_company_invoice_reference"
	else:
...
def make_maintenance_schedule(source_name, target_doc=None):
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Quick Sales Invoice": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
			"Quick Sales Invoice Item": {
...
def make_delivery_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Quick Sales Invoice": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
			"Quick Sales Invoice Item": {
...
def make_sales_return(source_name, target_doc=None):
	...
	return make_return_doc("Quick Sales Invoice", source_name, target_doc)


def get_inter_company_details(doc, doctype):
	if doctype in ["Quick Sales Invoice", "Sales Order", "Delivery Note"]:
...
def validate_inter_company_transaction(doc, doctype):
	...
	if not party:
		partytype = "Supplier" if doctype in ["Quick Sales Invoice", "Sales Order"] else "Customer"
...
def make_inter_company_purchase_invoice(source_name, target_doc=None):
	return make_inter_company_transaction("Quick Sales Invoice", source_name, target_doc)


def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Quick Sales Invoice", "Sales Order"]:
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Purchase Invoice" if doctype == "Quick Sales Invoice" else "Purchase Order"
		target_detail_field = "sales_invoice_item" if doctype == "Quick Sales Invoice" else "sales_order_item"
		source_document_warehouse_field = "target_warehouse"
		target_document_warehouse_field = "from_warehouse"
		received_items = get_received_items(source_name, target_doctype, target_detail_field)
	else:
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Quick Sales Invoice" if doctype == "Purchase Invoice" else "Sales Order"
...
def create_invoice_discounting(source_name, target_doc=None):
	invoice = frappe.get_doc("Quick Sales Invoice", source_name)
