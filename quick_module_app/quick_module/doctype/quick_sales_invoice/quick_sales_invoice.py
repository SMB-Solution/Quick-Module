...
class QuickSalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		advances: DF.Table[QuickSalesInvoiceAdvance]
		...
		is_pos: DF.Check
		is_return: DF.Check
		items: DF.Table[QuickSalesInvoiceItem]
		...
		payments: DF.Table[QuickSalesInvoicePayment]
	...
	def on_submit(self):
		...
		if not cint(self.is_pos) == 1 and not self.is_return and self.is_return:
