...
// frappe.ui.form.on("Quick Quick Sales Invoice", {
// 	refresh(frm) {
...
erpnext.accounts.taxes.setup_tax_validations("Quick Sales Invoice");
erpnext.accounts.payment_triggers.setup("Quick Sales Invoice");
erpnext.accounts.pos.setup("Quick Sales Invoice");
...
erpnext.accounts.SalesInvoiceController = class SalesInvoiceController extends (
    ...
    refresh(doc, dt, dn) {
        ...
        if (this.frm.doc.is_return) {
            this.frm.return_print_format = "Quick Sales Invoice Return";
        }
        ...
        if (doc.docstatus == 1 && !doc.is_return) {
            ...
            if (cint(doc.update_stock) != 1) {
                // show Make Delivery Note button only if Quick Sales Invoice is not created from Delivery Note
...
frappe.ui.form.on("Quick Sales Invoice", {
    setup: function (frm) {
        ...
        var paymentDueDate = year + "-" + month + "-" + day
        frm.set_value("due_date", paymentDueDate);
        ...
        (frm.custom_make_buttons = {
            ...
            "Quick Sales Invoice": "Return / Credit Note",
    ...
    refresh: function (frm) {
        if (!cur_frm.doc.__islocal) {
            frm.add_custom_button('Create New Sales Invoice', function () {
                frappe.new_doc('Quick Sales Invoice');
            })
        }
...
frappe.ui.form.on('Quick Sales Invoice Item', {
    amount: function (frm) {
        frm.doc.items.forEach(function (row) {
            if (row.amount && row.amount !== (row.rate * row.qty)) {
